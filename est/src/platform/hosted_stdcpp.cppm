export module est:platform.hosted_stdcpp;

import std;
import :loop;
import :platform;

// The one concrete platform::interface implementation that exists so far -
// std::chrono for the clock, std::this_thread for sleeping, std::cerr for
// diagnostics, std::abort() for a failed check. See :platform's own top
// comment for why this lives in its own module rather than alongside
// interface itself: this class needs to name est::loop (get_current_loop_
// context()'s own lazily-constructed fallback, below), and :platform sits
// below :loop in the dependency DAG specifically so it never has to.
...
public:
  ...
  // this hosted_stdcpp instance does (this codebase's normal, single
