...
// file's job (per review: `import est;` doesn't install one either,
// exactly like examples/hello_world/main.cpp and
// examples/sleep_sort/main.cpp each do their own installation in their
// own main()). This file is the test binary's equivalent of that main() -
// linked against plain Catch2::Catch2 (not Catch2::Catch2WithMain,
// est/tests/CMakeLists.txt) so this can supply its own, installing
// hosted_stdcpp exactly once before any TEST_CASE runs.
//
