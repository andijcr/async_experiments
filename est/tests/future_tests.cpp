...
// --- future<T>::take()/est::extraction<T> (issue #116) ---
//
// take() is pure sugar over std::move(*this).get() (its own doc comment),
// so it inherits every existing get() guarantee unchanged - these tests
// only check the parts specific to take()/extraction<T> itself (the
// wrapping, and that it still works for a move-only T); the underlying
// move-vs-copy dispatch and move-only-T support are already covered by
// get()'s own tests above and the "Move-only T" section further up this
// file. The actual clang [[consumable]] enforcement (double
// std::move(extraction).value(), or using one after it's been moved-from)
// is a compile-time -Wconsumed diagnostic, not something a runtime
// Catch2 assertion can observe - verified directly against the pinned
// Clang 22 during issue #116's own investigation, not re-proven here.
//
// A real gotcha found while writing these, worth recording for whoever
// adds the next one: -Wconsumed false-positives ("invalid invocation ...
// while it is in the 'consumed' state") on a *single*, genuinely correct
// std::move(extraction_var).value() call when it's written directly
// inside a Catch2 REQUIRE(...)/CHECK(...) argument and `extraction_var`
// is a named variable - Catch2's own expression-decomposition macros
// appear to reference a named sub-expression's type more than once
// (template/decltype machinery for its "print the LHS/RHS on failure"
// support), which is enough to confuse a syntactic, AST-level analysis
// like this one even though the call only ever executes once at runtime.
// An unnamed, single-chain call (std::move(fut).take().value(), no
// intermediate variable at all - the first test below) doesn't trigger
// it. Workaround: extract via .value() in its own plain statement first,
// then REQUIRE()/CHECK() on the already-extracted result, as the second
// test below does - never call .value() on a named extraction<T> from
// inside the macro argument itself.

TEST_CASE("take().value() extracts the value, sole owner", "[future][take]") {
  est::loop loop;
  const auto loop_guard = est::make_current_loop(loop);
  auto [promise, future] = est::make_promise_future<int>();
  promise.set_value(42);
  ...
  // NOLINTNEXTLINE(bugprone-use-after-move) - false positive, see the
  // unique_ptr flatten test's own NOLINTNEXTLINE comment above.
  REQUIRE(std::move(future).take().value() == 42);
}

TEST_CASE("take() works for a move-only T", "[future][take][move-only]") {
  est::loop loop;
  const auto loop_guard = est::make_current_loop(loop);
  auto [promise, future] = est::make_promise_future<std::unique_ptr<int>>();
  promise.set_value(std::make_unique<int>(7));

  auto extracted = std::move(future).take();
  auto value = std::move(extracted).value();
  REQUIRE(*value == 7);
}

TEST_CASE("take() rethrows a stored exception exactly like get() does, without wrapping it",
          "[future][take]") {
  est::loop loop;
  const auto loop_guard = est::make_current_loop(loop);
  auto [promise, future] = est::make_promise_future<int>();
  ...
  REQUIRE_THROWS_AS(std::move(future).take(), std::runtime_error);
}

// No test for future<void>::take() not existing: `requires(!std::is_void_v<T>)`
// on an ordinary (non-template) member function of a class template isn't
// SFINAE-friendly inside a requires(){...} expression on this toolchain -
// verified directly (a minimal reproduction hard-errors with "invalid
// reference to function... constraints not satisfied" instead of making
// the requires-expression false) - the same "this specific compile-time
// check isn't mechanically testable, even though the runtime behavior it
// guards is real and correct" lesson issue #117/PR #118 already ran into
// for then()/then_fast()'s own deduced return type, a different mechanism
// hitting the identical outcome here.
