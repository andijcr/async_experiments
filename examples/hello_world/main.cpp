...
auto main() -> int {
  ...
  // make; est itself doesn't even know estext exists). `estext::
  // hosted_stdcpp` is the one that exists for a hosted program like this
  // one; `platform_instance` and `platform_guard` both need to outlive
  // everything below that might touch est::check()/est::loop's own
  // platform hooks, so they're declared first, right at the top of
  // main().
