...
| `est::spawn()` (no `loop&` parameter), `est::spawn_exception_hook_type`, `default_spawn_exception_hook()`, `est::set_spawn_exception_hook()`/`est::spawn_exception_hook()` (both `thread_local`, `est:spawn`-owned) | `est/src/spawn.cppm` |
