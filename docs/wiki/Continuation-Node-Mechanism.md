...
### `future<T>::take()`/`est::extraction<T>`: a real, compile-time-checked single-extraction guarantee (issue #116)

`get()` (above) has no compile-time enforcement against being called more
than once - calling it twice on the same still-valid handle is a logic bug
(the second call observes a moved-from value, or a stale copy), not
something the type system catches. Issue #116 asked whether `get()` itself
should be redesigned to be "safe by default." Investigating that surfaced
a real tension worth recording: once a `get()`-like accessor silently falls
back to a copy whenever it isn't the sole owner (the issue's own proposed
fix), "was this called twice" stops being a question a *static* analysis
can answer at all - it depends on a **runtime** fact (the live ref count)
a compile-time typestate check has no way to see. Static enforcement and
"safe by default" pull in opposite directions for this specific hazard;
you can have one or the other, not both from the same mechanism.

`take()`/`extraction<T>` take the other side of that tradeoff: `get()`
itself stays exactly as it was (unchanged by any of this), and a new,
purely additive method - `future<T>::take() &&` - hands back a small,
move-only, single-use wrapper instead of `T` directly:

```cpp
template <class T>
class [[clang::consumable(unconsumed)]] extraction {
public:
  [[clang::return_typestate(unconsumed)]]
  explicit extraction(T value) : value_(std::move(value)) {}
...
`take()` is pure sugar over `std::move(*this).get()` - it inherits every
guarantee `get()` already has (the move-vs-copy dispatch above, move-only
`T` support) unchanged, wrapping the result rather than doing anything new.
`value()` is what actually carries the guarantee: under `-Wconsumed`
(enabled repo-wide, `cmake/CompilerWarnings.cmake` - narrow in practice,
since `extraction<T>` is the only `[[clang::consumable]]`-annotated type
in this codebase, so the warning can only ever fire against callers of
this one type), calling `value()` twice, or on an `extraction<T>` already
moved away from, is a compile error, not a silent logic bug - verified
directly (both against minimal reproductions and against a deliberately
reintroduced double-`.value()` bug in this exact file, which failed to
compile with exactly the expected diagnostic before being reverted).

`[[clang::consumable]]` instead of deducing-this, deliberately:
Clang's consumed-analysis attributes (`callable_when`/`set_typestate`)
verifiably do not track state through an explicit object parameter at all
(tested directly - a deducing-this `get(this Self&& self)` shape produces
*zero* `-Wconsumed` diagnostics on the identical double-call pattern that
a classic `&&`-qualified method catches correctly). `value()` is therefore
the one method in `est:future` that deliberately isn't deducing-this -
the whole reason `take()` exists as a *separate*, small type rather than
changing `get()` in place: it keeps this one exception contained to a
single, tiny, dedicated class instead of forcing `future<T>` itself to
give up deducing-this everywhere.

A real, narrow gotcha found while writing the regression tests: a single,
genuinely correct `std::move(extraction_var).value()` call produces a
`-Wconsumed` false positive when written directly inside a Catch2
`REQUIRE(...)`/`CHECK(...)` argument, if `extraction_var` is a named
variable - Catch2's own expression-decomposition macros appear to
reference a named sub-expression's type more than once (template/decltype
machinery supporting its "print the LHS/RHS on failure" behavior), enough
to confuse this syntactic, AST-level analysis even though the call only
ever executes once at runtime. An unnamed, single-chain call
(`std::move(fut).take().value()`, no intermediate variable) doesn't
trigger it. `est/tests/future_tests.cpp`'s own `take()` tests extract via
`.value()` in its own plain statement first, then `REQUIRE()` on the
already-extracted result, rather than ever calling `.value()` on a named
`extraction<T>` from inside a Catch2 macro argument.

`future<T>::take()` doesn't exist for `T = void`
(`requires(!std::is_void_v<T>)`) - a `future<void>` carries no value to
guard a double extraction of in the first place. Not asserted on with
`static_assert(!requires(...))` in the test file - verified directly that,
on this toolchain, a `requires`-clause on an *ordinary* (non-template)
member function of a class template isn't SFINAE-friendly inside a
`requires(){...}` expression even with an explicit (non-deduced) return
type, a different mechanism than the deduced-`auto`-return-type case just
above hitting the identical "not mechanically testable this way, even
though the guarded behavior is real and correct" outcome.
